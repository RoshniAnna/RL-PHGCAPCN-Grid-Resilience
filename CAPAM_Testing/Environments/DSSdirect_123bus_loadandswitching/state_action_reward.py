...
from Environments.DSSdirect_123bus_loadandswitching.topology import *
import scipy as sp
...
"""
COMPUTE HIGHER ORDER LAPLACIANS
To do Jaidev : Add comments. 

"""
def get_edge_index(edges, u, v):
    try:
        return edges.index((u, v))
    except ValueError:
        return edges.index((v, u))

def compute_hodge_laplacian(G, node_features):
    ...
    W1 = np.zeros((n_edges, n_edges))
    for idx, (i, j) in enumerate(edges):
        weight = np.sum((node_features[i] - node_features[j])**2)
        W1[idx, idx] = np.exp(-weight)
    W1 = sparse.diags(W1.diagonal())
    ...
    if triangles:
        B2 = np.zeros((n_edges, len(triangles)))
        for t_idx, (i, j, k) in enumerate(triangles):
            e1 = get_edge_index(edges, i, j)
            e2 = get_edge_index(edges, j, k)
            e3 = get_edge_index(edges, i, k)
            B2[e1, t_idx] = 1
            B2[e2, t_idx] = 1
            B2[e3, t_idx] = -1
        B2 = sparse.csr_matrix(B2)
        
        W2 = sparse.eye(len(triangles))
        L1 = W1 @ B1.T @ B1 @ W1 + B2 @ W2 @ B2.T
    else:
        L1 = W1 @ B1.T @ B1 @ W1
    
    return L0, L1

def get_hodge_decomposition(G, node_features):
    L0, L1 = compute_hodge_laplacian(G, node_features)
    return L0, L1
...
def get_state(DSSCktobj, G, edgesout):
    ...
    # print("Computing Hodge Laplacian")
    # L0, L1 = get_hodge_decomposition(G=nx.from_numpy_array(np.array(Adj_mat.todense())), 
    #                                  node_features=np.array(Vmagpu))
    # Topo_Laplacian = get_topo_laplacian(np.array(Vmagpu), np.array(Adj_mat.todense()))
...
def get_topo_laplacian(node_props, adj_):
    ...
    secondorder_subgraph = k_th_order_weighted_subgraph(adj_mat=adj_array, w_adj_mat=w_adj*adj_, k=2)
...
def var_preprocess(adj, r):
    ...
    adj_ = np.power(adj_, r) #adj_ = adj_ ** r
    adj_[adj_ > 1] = 1
    
    adj_sparse = sp.sparse.csr_matrix(adj_)
    
    rowsum = np.array(adj_sparse.sum(1)).ravel() #rowsum = np.ravel(adj_.sum(1))
    # rowsum = adj_.sum(1).A1
    degree_mat_inv_sqrt = sp.sparse.diags(np.power(rowsum, -0.5)) #degree_mat_inv_sqrt = sp.sparse.diags(np.power(rowsum, -0.5))
    adj_normalized = degree_mat_inv_sqrt @ adj_sparse @ degree_mat_inv_sqrt #adj_normalized = adj_.dot(degree_mat_inv_sqrt).T.dot(degree_mat_inv_sqrt).tocsr()
